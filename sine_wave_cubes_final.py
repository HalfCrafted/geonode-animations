...
def build_sine_wave_cubes_tree(ng):
    """Constructs the node tree for the sine wave cubes effect, traversing the full sphere."""
    ...
    socket(iface, name="Duration (s)", in_out='INPUT', socket_type='NodeSocketFloat', desc="How long the pole-to-pole animation takes", default=6.0, min_val=0.1, max_val=300.0)
    ...
    # 2. Pole-to-pole path generation logic
    map_angle = new_node(nodes, 'ShaderNodeMapRange', location=(-1400, 100)); map_angle.clamp = True
    map_angle.inputs['To Max'].default_value = math.pi
    links.new(time_val.outputs['Value'], map_angle.inputs['Value']); links.new(n_in.outputs['Duration (s)'], map_angle.inputs['From Max'])
    phi_angle = map_angle.outputs['Result']

    sin_phi = new_node(nodes, 'ShaderNodeMath', location=(-1200, 200)); sin_phi.operation = 'SINE'
    links.new(phi_angle, sin_phi.inputs[0])
    ring_radius = new_node(nodes, 'ShaderNodeMath', location=(-1000, 200)); ring_radius.operation = 'MULTIPLY'
    links.new(n_in.outputs['Sphere Radius'], ring_radius.inputs[0]); links.new(sin_phi.outputs['Value'], ring_radius.inputs[1])

    cos_phi = new_node(nodes, 'ShaderNodeMath', location=(-1200, 0)); cos_phi.operation = 'COSINE'
    links.new(phi_angle, cos_phi.inputs[0])
    ring_height_z = new_node(nodes, 'ShaderNodeMath', location=(-1000, 0)); ring_height_z.operation = 'MULTIPLY'
    links.new(n_in.outputs['Sphere Radius'], ring_height_z.inputs[0]); links.new(cos_phi.outputs['Value'], ring_height_z.inputs[1])

    path_circle = new_node(nodes, 'GeometryNodeCurvePrimitiveCircle', location=(-800, 200))
    links.new(ring_radius.outputs['Value'], path_circle.inputs['Radius'])
    height_vector = new_node(nodes, 'ShaderNodeCombineXYZ', location=(-600, 0))
    links.new(ring_height_z.outputs['Value'], height_vector.inputs['Z'])
    set_pos = new_node(nodes, 'GeometryNodeSetPosition', location=(-400, 100))
    links.new(path_circle.outputs['Curve'], set_pos.inputs['Geometry']); links.new(height_vector.outputs['Vector'], set_pos.inputs['Offset'])

    resample = new_node(nodes, 'GeometryNodeResampleCurve', location=(-200, 100)); resample.mode = 'COUNT'
    links.new(set_pos.outputs['Geometry'], resample.inputs['Curve']); links.new(n_in.outputs['Count'], resample.inputs['Count'])

    # 3. Calculate Sine Wave for Z-Scale
    idx = new_node(nodes, 'GeometryNodeInputIndex', location=(-200, -200))
    idx_div_count = new_node(nodes, 'ShaderNodeMath', location=(0, -200)); idx_div_count.operation = 'DIVIDE'
    ...
    mul_freq = new_node(nodes, 'ShaderNodeMath', location=(400, -200)); mul_freq.operation = 'MULTIPLY'
    links.new(mul_2pi.outputs['Value'], mul_freq.inputs[0]); links.new(n_in.outputs['Wave Frequency'], mul_freq.inputs[1])

    time_phase = new_node(nodes, 'ShaderNodeMath', location=(400, -350)); time_phase.operation = 'MULTIPLY'
    links.new(time_val.outputs['Value'], time_phase.inputs[0]); links.new(n_in.outputs['Wave Speed'], time_phase.inputs[1])
    add_phase = new_node(nodes, 'ShaderNodeMath', location=(600, -200)); add_phase.operation = 'ADD'
    links.new(mul_freq.outputs['Value'], add_phase.inputs[0]); links.new(time_phase.outputs['Value'], add_phase.inputs[1])

    wave_val = new_node(nodes, 'ShaderNodeMath', location=(800, -200)); wave_val.operation = 'SINE'
    links.new(add_phase.outputs['Value'], wave_val.inputs[0])

    map_height = new_node(nodes, 'ShaderNodeMapRange', location=(1000, -200))
    ...
    scale_vec = new_node(nodes, 'ShaderNodeCombineXYZ', location=(1000, -50))
    ...
    # --- GIMBAL LOCK PROTECTED ORIENTATION --- 
    ring_point_pos = new_node(nodes, 'GeometryNodeInputPosition', location=(0, 400))
    ...
    # Check if we are at the pole: abs(normal.z) > 0.9999
    abs_z = new_node(nodes, 'ShaderNodeMath', location=(400, 650)); abs_z.operation = 'ABSOLUTE'
    links.new(sep_norm.outputs['Z'], abs_z.inputs[0])
    is_at_pole = new_node(nodes, 'ShaderNodeMath', location=(600, 650)); is_at_pole.operation = 'GREATER_THAN'
    links.new(abs_z.outputs['Value'], is_at_pole.inputs[0]); is_at_pole.inputs[1].default_value = 0.9999

    # Calculate Z rotation (atan2)
    z_rot_raw = new_node(nodes, 'ShaderNodeMath', location=(600, 450)); z_rot_raw.operation = 'ARCTAN2'
    links.new(sep_norm.outputs['Y'], z_rot_raw.inputs[0]); links.new(sep_norm.outputs['X'], z_rot_raw.inputs[1])
    
    # Stable math-based switch: z_rot = (1 - is_at_pole) * z_rot_raw
    one_minus_factor = new_node(nodes, 'ShaderNodeMath', location=(800, 550)); one_minus_factor.operation = 'SUBTRACT'
    one_minus_factor.inputs[0].default_value = 1.0
    links.new(is_at_pole.outputs['Value'], one_minus_factor.inputs[1])

    z_rot = new_node(nodes, 'ShaderNodeMath', location=(1000, 450)); z_rot.operation = 'MULTIPLY'
    links.new(one_minus_factor.outputs['Value'], z_rot.inputs[0])
    links.new(z_rot_raw.outputs['Value'], z_rot.inputs[1])

    # Y rotation is safe
    y_rot = new_node(nodes, 'ShaderNodeMath', location=(800, 650)); y_rot.operation = 'ARCCOSINE'
    links.new(sep_norm.outputs['Z'], y_rot.inputs[0])

    # Combine to make final rotation vector
    rot_vec = new_node(nodes, 'ShaderNodeCombineXYZ', location=(1200, 500))
    links.new(y_rot.outputs['Value'], rot_vec.inputs['Y'])
    links.new(z_rot.outputs['Value'], rot_vec.inputs['Z'])
