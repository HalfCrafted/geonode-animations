# Blender 4.4 - "Square Wave Torus Effect" Geometry Nodes Script
# Creates a horizontal torus that expands, with a square wave displacing it.
...
def build_square_wave_torus_tree(ng):
    """Constructs the node tree for the square wave torus effect."""
    ...
    spline_param = new_node(nodes, 'GeometryNodeSplineParameter', location=(-1800, 400))
    
    # Wave Angle = (Factor * 2PI * Frequency) + (Time * Speed)
    factor_to_rad = new_node(nodes, 'ShaderNodeMath', location=(-1600, 400)); factor_to_rad.operation = 'MULTIPLY'; factor_to_rad.inputs[1].default_value = 2 * math.pi
    links.new(spline_param.outputs['Factor'], factor_to_rad.inputs[0])

    freq_mult = new_node(nodes, 'ShaderNodeMath', location=(-1400, 400)); freq_mult.operation = 'MULTIPLY'
    links.new(factor_to_rad.outputs['Value'], freq_mult.inputs[0])
    links.new(n_in.outputs['Wave Frequency'], freq_mult.inputs[1])

    time_mult = new_node(nodes, 'ShaderNodeMath', location=(-1400, 250)); time_mult.operation = 'MULTIPLY'
    links.new(time_val.outputs['Value'], time_mult.inputs[0])
    links.new(n_in.outputs['Wave Speed'], time_mult.inputs[1])

    add_phase = new_node(nodes, 'ShaderNodeMath', location=(-1200, 350)); add_phase.operation = 'ADD'
    links.new(freq_mult.outputs['Value'], add_phase.inputs[0])
    links.new(time_mult.outputs['Value'], add_phase.inputs[1])

    # Create Square wave: sign(sin(angle))
    wave_sine = new_node(nodes, 'ShaderNodeMath', location=(-1000, 350)); wave_sine.operation = 'SINE'
    links.new(add_phase.outputs['Value'], wave_sine.inputs[0])

    square_wave = new_node(nodes, 'ShaderNodeMath', location=(-800, 350)); square_wave.operation = 'SIGN'
    links.new(wave_sine.outputs['Value'], square_wave.inputs[0])

    # Final Wave Value = SquareWave * Amplitude
    wave_amp = new_node(nodes, 'ShaderNodeMath', location=(-600, 350)); wave_amp.operation = 'MULTIPLY'
    links.new(square_wave.outputs['Value'], wave_amp.inputs[0])
...
def setup_and_run():
    ...
    print("--- Starting Square Wave Torus Script ---")
    ...
    obj.name = "SquareWaveTorusObject"

    ng = new_gn_group("SquareWaveTorusTree")
    build_square_wave_torus_tree(ng)
    
    mod = attach_group_modifier(obj, ng, "SquareWaveTorusModifier")

    print(f"Successfully created and assigned '{ng.name}' to modifier '{mod.name}' on object '{obj.name}'.")
    print("--- Square Wave Torus Script Finished ---")
