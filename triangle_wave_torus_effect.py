# Blender 4.4 - "Triangle Wave Torus Effect" Geometry Nodes Script
# Creates a horizontal torus that expands, with a triangle wave displacing it.
...
def build_triangle_wave_torus_tree(ng):
    """Constructs the node tree for the triangle wave torus effect."""
    ...
    n_in = new_node(nodes, 'NodeGroupInput', location=(-1800, 0))
    n_out = new_node(nodes, 'NodeGroupOutput', location=(800, 100))

    # Time driver
    time_val = new_node(nodes, 'ShaderNodeValue', location=(-1800, -300)); time_val.label = "Time (s)"
    safe_time_driver(time_val)

    # Animate the major radius
    map_radius = new_node(nodes, 'ShaderNodeMapRange', location=(-1600, 100)); map_radius.clamp = True
    ...
    path_circle = new_node(nodes, 'GeometryNodeCurvePrimitiveCircle', location=(-1400, 100))
    ...
    set_pos = new_node(nodes, 'GeometryNodeSetPosition', location=(-1200, 100))
    links.new(path_circle.outputs['Curve'], set_pos.inputs['Geometry'])

    spline_param = new_node(nodes, 'GeometryNodeSplineParameter', location=(-2000, 400))
    
    # Wave Angle = (Factor * 2PI * Frequency) + (Time * Speed)
    factor_to_rad = new_node(nodes, 'ShaderNodeMath', location=(-1800, 400)); factor_to_rad.operation = 'MULTIPLY'; factor_to_rad.inputs[1].default_value = 2 * math.pi
    links.new(spline_param.outputs['Factor'], factor_to_rad.inputs[0])

    freq_mult = new_node(nodes, 'ShaderNodeMath', location=(-1600, 400)); freq_mult.operation = 'MULTIPLY'
    links.new(factor_to_rad.outputs['Value'], freq_mult.inputs[0])
    links.new(n_in.outputs['Wave Frequency'], freq_mult.inputs[1])

    time_mult = new_node(nodes, 'ShaderNodeMath', location=(-1600, 250)); time_mult.operation = 'MULTIPLY'
    links.new(time_val.outputs['Value'], time_mult.inputs[0])
    links.new(n_in.outputs['Wave Speed'], time_mult.inputs[1])

    add_phase = new_node(nodes, 'ShaderNodeMath', location=(-1400, 350)); add_phase.operation = 'ADD'
    links.new(freq_mult.outputs['Value'], add_phase.inputs[0])
    links.new(time_mult.outputs['Value'], add_phase.inputs[1])

    # Create Triangle wave: arcsin(sin(angle))
    wave_sine = new_node(nodes, 'ShaderNodeMath', location=(-1200, 350)); wave_sine.operation = 'SINE'
    links.new(add_phase.outputs['Value'], wave_sine.inputs[0])

    triangle_wave = new_node(nodes, 'ShaderNodeMath', location=(-1000, 350)); triangle_wave.operation = 'ARCSINE'
    links.new(wave_sine.outputs['Value'], triangle_wave.inputs[0])

    # Map the [-PI/2, PI/2] output of Arcsine to [-1, 1]
    map_wave = new_node(nodes, 'ShaderNodeMapRange', location=(-800, 350))
    map_wave.inputs['From Min'].default_value = -math.pi / 2
    map_wave.inputs['From Max'].default_value = math.pi / 2
    map_wave.inputs['To Min'].default_value = -1.0
    map_wave.inputs['To Max'].default_value = 1.0
    links.new(triangle_wave.outputs['Value'], map_wave.inputs['Value'])

    # Final Wave Value = MappedTriangleWave * Amplitude
    wave_amp = new_node(nodes, 'ShaderNodeMath', location=(-600, 350)); wave_amp.operation = 'MULTIPLY'
    links.new(map_wave.outputs['Result'], wave_amp.inputs[0])
    links.new(n_in.outputs['Wave Amplitude'], wave_amp.inputs[1])

    # Create Z-axis displacement vector and apply it
    disp_vec = new_node(nodes, 'ShaderNodeCombineXYZ', location=(-1000, 100))
    ...
    profile_circle = new_node(nodes, 'GeometryNodeCurvePrimitiveCircle', location=(-1000, -100))
    ...
    curve_to_mesh = new_node(nodes, 'GeometryNodeCurveToMesh', location=(-800, 100))
...
def setup_and_run():
    ...
    print("--- Starting Triangle Wave Torus Script ---")
    ...
    obj.name = "TriangleWaveTorusObject"

    ng = new_gn_group("TriangleWaveTorusTree")
    build_triangle_wave_torus_tree(ng)
    
    mod = attach_group_modifier(obj, ng, "TriangleWaveTorusModifier")

    print(f"Successfully created and assigned '{ng.name}' to modifier '{mod.name}' on object '{obj.name}'.")
    print("--- Triangle Wave Torus Script Finished ---")
