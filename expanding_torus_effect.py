# Blender 4.4 - "Expanding Torus Effect" Geometry Nodes Script
# Creates a horizontal torus that expands its main radius over time while keeping its thickness constant.
...
def build_expanding_torus_tree(ng):
    """Constructs the node tree for the expanding torus effect."""
    nodes, links, iface = ng.nodes, ng.links, ng.interface

    # ---- 1. Interface Definition ----
    socket(iface, name="Start Radius", in_out='INPUT', socket_type='NodeSocketFloat', desc="Major radius of the torus at the start", default=0.5, min_val=0.0, max_val=50.0)
    socket(iface, name="End Radius", in_out='INPUT', socket_type='NodeSocketFloat', desc="Major radius of the torus at the end", default=5.0, min_val=0.0, max_val=50.0)
    socket(iface, name="Duration (s)", in_out='INPUT', socket_type='NodeSocketFloat', desc="Time for the expansion animation", default=4.0, min_val=0.1, max_val=100.0)
    socket(iface, name="Torus Thickness", in_out='INPUT', socket_type='NodeSocketFloat', desc="Thickness (minor radius) of the torus ring", default=0.25, min_val=0.01, max_val=10.0)
    socket(iface, name="Path Resolution", in_out='INPUT', socket_type='NodeSocketInt', desc="Resolution of the main torus path", default=128, min_val=3, max_val=512)
    socket(iface, name="Profile Resolution", in_out='INPUT', socket_type='NodeSocketInt', desc="Resolution of the torus thickness profile", default=32, min_val=3, max_val=256)
    ...
    n_out = new_node(nodes, 'NodeGroupOutput', location=(600, 100))
    ...
    # Create the main path circle
    path_circle = new_node(nodes, 'GeometryNodeCurvePrimitiveCircle', location=(-800, 100))
    ...
    profile_circle = new_node(nodes, 'GeometryNodeCurvePrimitiveCircle', location=(-600, -100))
    ...
    curve_to_mesh = new_node(nodes, 'GeometryNodeCurveToMesh', location=(-400, 100))
...
def setup_and_run():
    ...
    print("--- Starting Expanding Torus Script ---")
    ...
    obj.name = "TorusObject"

    # Create and build the node group
    ng = new_gn_group("ExpandingTorusTree")
    build_expanding_torus_tree(ng)
    
    # Attach the modifier
    mod = attach_group_modifier(obj, ng, "ExpandingTorusModifier")

    print(f"Successfully created and assigned '{ng.name}' to modifier '{mod.name}' on object '{obj.name}'.")
    print("--- Expanding Torus Script Finished ---")
